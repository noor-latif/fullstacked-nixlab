...
- **Migration path (REVISED 2026-09-11): sub-zone delegation is IMPOSSIBLE — DeSEC must be authoritative for `fullstacked.se`.** Stalwart has no HostUp provider and HostUp's API has no TSIG/RFC2136 primitive — only HTTPS record CRUD for hook-style clients. HTTP-01 is impossible (Caddy reserves `/.well-known/acme-challenge/*` for its own ACME — user routes for that path never fire, verified: 308 + `looking up info for HTTP challenge` in Caddy log); TLS-ALPN-01 needs port 443 which is Caddy's (stock Caddy terminates TLS itself; only a custom caddy-l4 build could forward `acme-tls/1` by ALPN — rejected as fragile). Plan A (APPROVED + EXECUTING 2026-09-11): DeSEC hosts `fullstacked.se`. Zonefile for manual dashboard import saved at `/home/noor/fullstacked.se.zone` (31 live records; verified serving from ns1.desec.io before the swap). Delegation change to ns1.desec.io/ns2.desec.org ACCEPTED by registry via MCP `update_domain_nameservers` 2026-09-11 (~minutes to propagate). DS auto-removal on NS change per KB #832118; SmartDNSSEC Sync re-adds DeSEC DS after mandatory 24h block per KB #958624 (DeSEC coverage unconfirmed — manual fallback DS: tag 56980 alg 13 digest-type 2 digest 3e698f3f65d249b336fa66c6a84a216c7aca0b344127686b1f507b4d84c3944d). WATCHER ACTIVE: `scripts/desec-delegation-watch.py` + user systemd timer `desec-watch.timer` on nixlab (every 6h x 8 runs = 48h, linger on; mails noor.crystal@gmail.com via health-account SMTP on transitions: NS flipped / DS back / SUCCESS / FAIL). After all-green: create `DnsServer/DeSEC` object (`DESEC_API_TOKEN` confirmed on nixlab 2026-09-11, zone visible) → confirm one renewal → delete Cloudflare `DnsServer` + disable timer. `acme.fullstacked.se` DeSEC zone deleted by user 2026-09-11 (unneeded).
- HostUp v2 ACME-relevant endpoints (read from lego's `hostup` provider source 2026-09-11; NOT usable by Stalwart, recorded for hook-script fallback only): base `https://cloud.hostup.se/api/v2`, `Authorization: Bearer <key>`; `GET /dns-zones?name=<zone>` → zone id; `POST /dns-zones/{id}/records` `{type:TXT,name,value,ttl}` → record id; `DELETE /dns-zones/{zoneID}/records/{recordID}`. Their own house solution is hook scripts, NOT TSIG: official acme.sh plugin at `github.com/hostup/acme.sh` (`dnsapi/dns_hostup.sh`, MERGED upstream — verified live in acmesh-official/acme.sh master, newer than the fork: v2 endpoints, TTL 60–86400 validation), KB precedents ticket #763670 (customer asked for Certbot plugin or RFC2136 → got custom acme.sh script + dns-scoped key) and #386405 (Certbot manual hook + `_acme-challenge` API guide). Registrar NS change is API-doable (`POST /api/v2/domains/{id}/nameservers`, MCP `update_domain_nameservers`). Feedback filed 2026-09-11: TSIG-or-driver request + follow-up noting DNS move to DeSEC (via MCP `submit_feedback`). Stalwart upstream issue `stalwartlabs/stalwart#3310` filed 2026-09-11 (HostUp provider request, full API spec + prior art). Rejected paths: external lego (nixpkgs ships lego 4.35.0-ish, provider needs 5.0.0 → source-build overlay required; plus cert-import-plus-restart glue per renewal). CNAME-delegation design DEAD (verified in Stalwart source 2026-09-11: `order.rs` writes `_acme-challenge.<domain>` literally, `dns-update/desec.rs` `discover_domain` only walks UP — a DeSEC-hosted child zone can never receive the write).
