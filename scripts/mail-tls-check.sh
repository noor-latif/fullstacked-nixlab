...
for spec in "25:smtp" "465:" "993:"; do
  ...
  if [ -z "$days" ]; then echo "$BAD  $MAILHOST:$p  no cert / handshake failed"; else
...
if [ -z "$days" ]; then echo "$BAD  $DOMAIN:443  no cert"; else
...
check_txt() { # name expected-substring
  ...
  if [ -z "$got" ]; then echo "$BAD  $name  no TXT record"; return; fi
...
check_a() { # name expected-ip
  ...
  if [ "$got" = "$want" ]; then echo "$OK  $name  -> $got"; else
    echo "$BAD  $name  -> ${got:-<none>}  (expected $want)"; fi
}
check_resolves() { # name (informational; must resolve to an IP)
  local name="$1" got
  got=$(dig +short A "$name" @"$CF_RESOLVER" 2>/dev/null | tail -1)
  if [ -n "$got" ]; then echo "$OK  $name  -> $got"; else echo "$BAD  $name  -> <none>"; fi
}
