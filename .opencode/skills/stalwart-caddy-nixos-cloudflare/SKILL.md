...
- HostUp support verdict (2026-09-10, via their agent): no TSIG/RFC2136, period. They DO have a v2 REST API (`/api/v2/dns-zones/{id}/records`) with scoped keys — but it only fits hook-style clients (acme.sh/Certbot hook scripts). Stalwart's built-in ACME client cannot call external hooks, so the v2 API can't serve Stalwart renewals — DeSEC delegation stands. (Declined their cert-manager webhook follow-up: no Kubernetes here.) The v2 API remains useful for future scripted DNS work with a DNS-scoped key.
