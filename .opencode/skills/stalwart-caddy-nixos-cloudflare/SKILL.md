...
- **Migration path (REVISED 2026-09-11): sub-zone delegation is IMPOSSIBLE — DeSEC must be authoritative for `fullstacked.se`.** Stalwart has no HostUp provider and HostUp's API has no TSIG/RFC2136 primitive — only HTTPS record CRUD for hook-style clients. HTTP-01 is impossible (Caddy reserves `/.well-known/acme-challenge/*` for its own ACME — user routes for that path never fire, verified: 308 + `looking up info for HTTP challenge` in Caddy log); TLS-ALPN-01 needs port 443 which is Caddy's (stock Caddy terminates TLS itself; only a custom caddy-l4 build could forward `acme-tls/1` by ALPN — rejected as fragile). Plan A (recommended): DeSEC hosts `fullstacked.se` (free account, recreate full record set from the known-good list, registrar NS+DS swap at HostUp-as-registrar — API-doable via `update_domain_nameservers`; zone has signed delegation so sequence NS+DS together) → `DnsServer/DeSEC` object → renewal works natively. Needs user go-ahead + `DESEC_API_TOKEN` on NIXLAB (was placed on laptop 2026-09-11, removed; not yet on nixlab). Then confirm one renewal, delete the Cloudflare `DnsServer` object.
- HostUp v2 ACME-relevant endpoints (read from lego's `hostup` provider source 2026-09-11; NOT usable by Stalwart, recorded for hook-script fallback only): base `https://cloud.hostup.se/api/v2`, `Authorization: Bearer <key>`; `GET /dns-zones?name=<zone>` → zone id; `POST /dns-zones/{id}/records` `{type:TXT,name,value,ttl}` → record id; `DELETE /dns-zones/{zoneID}/records/{recordID}`. Their own house solution is hook scripts, NOT TSIG: official acme.sh plugin at `github.com/hostup/acme.sh` (`dnsapi/dns_hostup.sh`), KB precedents ticket #763670 (customer asked for Certbot plugin or RFC2136 → got custom acme.sh script + dns-scoped key) and #386405 (Certbot manual hook + `_acme-challenge` API guide). Registrar NS change is API-doable (`POST /api/v2/domains/{id}/nameservers`, MCP `update_domain_nameservers`) — useful for a future DeSEC-authoritative move. Rejected paths: external lego (nixpkgs ships lego 4.35.2, provider needs 5.0.0 → source-build overlay required; plus cert-import-plus-restart glue per renewal), Stalwart-native driver (needs upstream Rust PR; no existing issue in stalwartlabs/stalwart as of 2026-09-11 — file one if pursuing). CNAME-delegation design DEAD (verified in Stalwart source 2026-09-11: `order.rs` writes `_acme-challenge.<domain>` literally, `dns-update/desec.rs` `discover_domain` only walks UP — a DeSEC-hosted child zone can never receive the write).
