...
2. **Pangolin mta-sts/autoconfig resources — DONE (2026-08-16).** Root cause of the earlier untrusted cert was a GLOBAL default TLS cert (minica self-signed) in `/opt/pangolin/config/traefik/dynamic_config.yml` (SANs only `mail`/`mta-sts`/`autoconfig`) that shadowed on-demand ACME. Removed that `tls: certificates:` block; on-demand ACME now issues real LE certs. `autodiscover.fullstacked.se` resource created (id `17`, target `17` → `172.18.0.1:1080`). `mta-sts`/`autoconfig` resources exist (targets originally `172.18.0.1:1080`).
 3. **RESOLVED — mta-sts/autoconfig 502 (2026-08-16).** Root cause: `SystemSettings.proxyTrustedNetworks = 172.18.0.0/16` populated the HTTP listener's `proxy_networks`, making Stalwart **require the PROXY protocol header** from Gerbil/Pangolin (`172.18.0.2`/`.3`) while exempting the bind IP (`172.18.0.1` = host → `200`). Gerbil sent no PROXY header → silent close → `502`. **Fix applied:** set `proxyTrustedNetworks = {}` and restarted Stalwart → external `mta-sts`/`autoconfig` now `200`, `autodiscover`/`mail` `302` (Traefik SSO redirect, by design). **Follow-up (proper XFF trust):** to restore real client-IP for HTTP bans, set `proxyProtocol = true` on listener `i31cxojoahqb` AND enable `proxyProtocol` on the Pangolin targets so Gerbil sends PROXY; otherwise keep `proxyTrustedNetworks` empty. **Cleanup still due:** delete duplicate Pangolin targets created during diagnosis — resource `1` has targetId `2`+`18`, resource `3` has `3`+`19` (delete `18`/`19` via `DELETE /target/{id}`).
