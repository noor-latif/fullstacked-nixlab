...
- **Migration path (REVISED 2026-09-11): sub-zone delegation is IMPOSSIBLE — DeSEC must be authoritative for `fullstacked.se`.** Stalwart has no HostUp provider and HostUp's API has no TSIG/RFC2136 primitive — only HTTPS record CRUD for hook-style clients. HTTP-01 is impossible (Caddy reserves `/.well-known/acme-challenge/*` for its own ACME — user routes for that path never fire, verified: 308 + `looking up info for HTTP challenge` in Caddy log); TLS-ALPN-01 needs port 443 which is Caddy's (stock Caddy terminates TLS itself; only a custom caddy-l4 build could forward `acme-tls/1` by ALPN — rejected as fragile). Plan A (APPROVED 2026-09-11): DeSEC hosts `fullstacked.se`. Zonefile for manual dashboard import saved at `/home/noor/fullstacked.se.zone` (31 live records: 5 A, 1 AAAA, 4 CNAME, 1 MX, 7 SRV verified prio 0 weight 1 via public DNS, 10 TXT with full live DKIM; SOA+apex NS omitted; TTL unified 3600 = DeSEC floor; v1-rsa retired). User imports manually, then: verify lookups → registrar NS+DS swap at HostUp-as-registrar (API-doable; signed delegation so sequence together) → `DnsServer/DeSEC` object → confirm one renewal → delete Cloudflare `DnsServer`. Still needs `DESEC_API_TOKEN` on NIXLAB (placed on laptop by mistake 2026-09-11, removed; not yet on nixlab — Stalwart's DeSEC object needs it server-side). `acme.fullstacked.se` DeSEC zone deleted by user 2026-09-11 (unneeded under plan A).
